...
- Strongly-typed `Input` structs reject wrong-shape, unknown, and misspelled input and return `CalcError::InvalidInput` rather than silently ignoring or coercing it; a registry policy test enforces every closed schema, and range/plausibility checks reject implausible values instead of scoring them (**C005, C007, C008**, addressing **H002, H003**).
