...
2. The engine's clipboard summary now carries the entered **inputs** and a `clincalc` version stamp alongside the result, interpretation, and reference (ENG-006.6, see H006); the host remains responsible for the durability of whatever record it pastes that summary into.
...
- `CalculationResponse` returns a human-readable `interpretation` and a `working` breakdown alongside every numeric `result`; the `reference` (primary citation), the entered inputs, and a `clincalc` version stamp all travel with the result in the clipboard summary (`to_summary_text`) and the CLI's text/Markdown output (**C013, C014**, addressing **H006**).
...
- **Resolved:** the clipboard summary (`to_summary_text`) and the CLI's text/Markdown output now also carry the entered inputs and a `clincalc` version stamp alongside result, interpretation, and reference (ENG-006.6), closing the gap previously recorded here and in H006's residual-risk table (§5). CSO to reassess H006's residual-risk score given this control is now implemented.
...
| H006 | 3 | 4 | medium | The engine never emits a naked number in its own output. The previously recommended improvement is now implemented: the clipboard summary (`to_summary_text`) and the CLI's text/Markdown output include the entered inputs and a `clincalc` version stamp, so a pasted result can be reconstructed from the text alone (ENG-006.6). Residual risk retained pending CSO review of whether this lowers the score; the remaining host obligation (deployment assumption 2) is the durability of the record it pastes into, not the presence of the inputs. |
...
*[PENDING - CSO judgement.]* On the evidence above, three principal residual risks are concentrated at the **host boundary** (units, optional-predicate semantics, and result provenance on copy-paste), while H009-H012 are calculator-specific risks of mistaking an estimate for a measurement, a preliminary observed maximum for a diagnostic threshold, a muscle-quantity threshold for a standalone sarcopenia diagnosis or exclusion, or an exploratory protein-intake breakpoint for an individual requirement or safety ceiling. Scoring itself is strongly controlled by primary-source verification, literature-vector testing, and mandatory evidenced provenance. The boundary risks are best closed by (a) the recommended `to_summary_text()` provenance improvement and (b) the corresponding DCB0160 deployer obligations; H009-H012 additionally depend on retaining each calculator's applicability restrictions and limitations wherever outputs are presented. The appointed CSO must make and record the overall judgement that the product, with these controls, is acceptably safe for its intended use - and should give particular attention, per calculator, to any high-stakes score whose worst-case severity exceeds the engine-wide assessment used here.
