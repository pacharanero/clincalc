...
The registry contains 11 confirmed rights-locked entries, three entries awaiting permission or legal review, and one safety-blocked entry. Confirmed rights-locked entries return `unavailable: proprietary`; NYHA, Norton, and SCORE2 / SCORE2-OP return `unavailable: rights-review` without asserting that independent implementation is prohibited. The safety-blocked SAD PERSONS entry returns `unavailable: clinical-safety`, the safety reason, cited evidence, and safer clinical approaches. The latter four are not described or tagged as proprietary, none returns a score, and the SAD PERSONS interpretation warns that it must not be used to reduce observation, referral, treatment, or admission/discharge decisions.
