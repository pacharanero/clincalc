...
The full registry: 114 entries comprising 100 active calculators that compute a real result, 11 named-but-unavailable rights-locked stubs (carrying `proprietary` and `unavailable`), two rights-review stubs, and one clinical-safety stub (the latter three carry `unavailable` without `proprietary`). One row per calculator.
...
| `skeletal_muscle_mass_index` | Skeletal Muscle Mass Index (SMI) | Appendicular lean mass normalised to height (EWGSOP2 SMI) and to BMI (FNIH ALM/BMI), each against its own consensus cut-off for low muscle quantity. Assesses muscle quantity only - not a standalone sarcopenia diagnosis, which also requires low muscle strength and, for severity, impaired physical performance. | `primary-care`, `geriatrics`, `musculoskeletal`, `screening` |
