...
The full registry: 115 entries comprising 101 active calculators that compute a real result, 11 named-but-unavailable rights-locked stubs (carrying `proprietary` and `unavailable`), two rights-review stubs, and one clinical-safety stub (the latter three carry `unavailable` without `proprietary`). One row per calculator.
...
| `body_adiposity_index` | Body Adiposity Index (BAI) | Legacy weight-free body-fat estimate from height and hip circumference (Bergman 2011), restricted to the external-validation cohort's adult measurement envelope. A 2018 systematic review found wide individual error, systematic overestimation at lower body fat and underestimation at higher body fat, and did not recommend BAI for adult body-fat determination. It is not a measurement, diagnosis, or treatment rule. | `primary-care`, `endocrinology`, `screening` |
...
| `fat_free_mass_index` | Fat-Free Mass Index (FFMI) | Fat-free mass relative to height from weight, height, and an independently obtained body fat percentage, including the Kouri 1995 normalization to a 1.8 m reference height. The paper's preliminary male-athlete sample maximum is not a diagnostic cut-point, biological limit, or proof of steroid use, and does not establish equivalent interpretation in other populations. | `primary-care`, `endocrinology`, `musculoskeletal`, `screening` |
...
| `nyha` | NYHA Functional Classification | Classifies current cardiac-symptom-related physical-activity limitation as Class I-IV using CC BY 4.0 NCI Thesaurus definitions. It is subjective, is not an objective exercise-capacity measure or treatment rule, and records the selected functional profile and NCI concept identifier with the result. | `cardiology`, `severity` |
...
| `protein_target` | Resistance-Training Protein Intake | Calculates Morton et al.'s prudent 2.2 g/kg/day protein amount for healthy, non-energy-restricted adults undertaking resistance training and reports the uncertain exploratory 1.62 g/kg/day breakpoint behind it. Not a general dietary minimum, a caloric-deficit target, or a safety ceiling. | `primary-care`, `endocrinology`, `musculoskeletal` |
...
| `skeletal_muscle_mass_index` | Skeletal Muscle Mass Index (SMI) | Whole-body DXA appendicular lean mass normalised to height (primary EWGSOP2 SMI result) and to BMI (supplementary FNIH preliminary candidate criterion) in adults aged 65 or older. Reports each cut-off classification separately because they can disagree. Assesses muscle quantity only - not a standalone sarcopenia diagnosis or exclusion. | `primary-care`, `geriatrics`, `musculoskeletal`, `screening` |
...
| **Energy-restricted protein / macronutrient target** | Protein or broader macronutrient targets from weight or lean body mass and a defined goal, including lean-mass retention during a caloric deficit. The shipped `protein_target` calculator covers only healthy, non-energy-restricted adults undertaking resistance training because Morton et al. excluded energy restriction. | `primary-care`, `endocrinology` |
