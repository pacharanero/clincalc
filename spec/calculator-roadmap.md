...
Shipped anthropometric and body-composition measures include BMI, body-fat circumference, BAI, WHtR, WHR, RFM, FFMI, 1RM, training zones, and the Wilks score. Remaining non-MedikQuantis candidates include lab or field methods for body composition (skinfolds, SMI) and the alternative DOTS strength score.
...
- [ ] **CALC-044 Energy-restricted protein / macronutrient target** - protein or broader macronutrient targets from weight or lean body mass and a defined goal, including lean-mass retention during a caloric deficit. The shipped `protein_target` calculator deliberately does not complete this candidate: Morton et al. excluded energy-restricted participants and supports only a total-body-weight protein amount for healthy adults undertaking resistance training.
...
- [ ] **CALC-048 Durnin-Womersley 4-site skinfold body fat %** - caliper-derived body-fat estimate for training and body-comparison settings, using the age-banded 4-site (biceps, triceps, subscapular, suprailiac) equations. The shipped `jackson_pollock_skinfold` calculator completes the 3-site Jackson-Pollock half of the original candidate; this item now tracks Durnin-Womersley only.
