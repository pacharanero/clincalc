...
Shipped anthropometric and body-composition measures include BMI, body-fat circumference, WHtR, WHR, RFM, 1RM, training zones, and the Wilks score. Remaining candidates under CALC-046..052 range from tape-measure proxies (BAI) to lab or field methods for body composition (skinfolds, FFMI, SMI) and the DOTS strength-scoring successor.
...
- [ ] **CALC-046 DOTS** - bodyweight-adjusted strength score (successor to Wilks, shipped separately as `wilks`); needs DOTS's polynomial coefficients confirmed against a citable primary source (IPF/USAPL technical documentation or the original derivation), not just secondary calculator sites, before implementation
