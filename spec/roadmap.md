...
- [x] **ENG-001.4 Enforce complete translation bundles** - the blocking registry test `every_advertised_locale_is_a_complete_bundle` (in `src/lib.rs`, backed by the schema-prose walker in `src/translation.rs`) fails any calculator advertising a locale whose localized schema is not at key parity with English, carries empty prose, or is byte-identical English. Computed prose stays under each calculator's own completeness tests (see `docs/translating.md`); the whole-representation English fallback was already the engine's default contract.
...
- [ ] **ENG-001.6 Wire locale through remaining surfaces** - CLI `--locale` / `CLINCALC_LOCALE`, the Python keyword-only `locale` (on `calculate`, `list_calculators`, `get_schema`, `get_template`, `batch`), and REST `?locale=` / `Accept-Language` / `Content-Language` / `Vary` / `clincalc api --locale` (`src/api.rs`) are implemented; MCP session configuration and GUI locale and text direction remain. All implemented surfaces report the resolved locale.
...
- [ ] **COLL-001.1 Verify point-for-point agreement** with the MedikQuantis implementation:
   ...
   Point values were confirmed 2026-07-28 against MedikQuantis's actual `cha2ds2vasc.ts` source (pinned at commit `cf5afb9`). Its public `en`/`es` UI copy and tests were checked 2026-08-24: they say only prior stroke/TIA and do not communicate or test systemic arterial thromboembolism. That omission is operationally important but does not prove the intended exclusion boundary. Author confirmation, an authoritative definition, or an explicit systemic-embolism boundary test is still needed. See "Cross-project verification" in `spec/calculators/cha2ds2-vasc.md`, which also documents the open recommendation-threshold divergence and citation mismatch.
