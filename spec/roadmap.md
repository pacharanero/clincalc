...
- [x] **COLL-001.1 Verify point-for-point agreement** with the MedikQuantis implementation:
   ...
   Confirmed 2026-07-28 against MedikQuantis's actual `cha2ds2vasc.ts` source (not just its description, pinned at commit `cf5afb9`) - point values agree on every criterion, including the S2 criterion's 2 points, though whether MedikQuantis's `strokeOrTia` field also captures systemic arterial thromboembolism specifically hasn't been confirmed (their field name suggests it may not). See "Cross-project verification" in `spec/calculators/cha2ds2-vasc.md`, which also documents a real divergence in the *recommendation thresholds* (NICE NG196 vs MedikQuantis's sex-specific scheme) surfaced by this comparison - not a scoring bug, but worth a decision under `ENG-001`.
...
- [x] **COLL-001.3 Add literature-vector tests** pinning the exact boundary cases Laura raised (age 74 vs 75) and the annual stroke-risk table from Friberg 2012 (PMID 22246443) to `src/calculators/cha2ds2vasc.rs`. Add the risk percentages to `working` so they are visible to both projects.
